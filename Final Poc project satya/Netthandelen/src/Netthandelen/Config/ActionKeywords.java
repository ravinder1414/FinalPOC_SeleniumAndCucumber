...
public class ActionKeywords {
	...
	static Logger loger = Logger.getLogger(ActionKeywords.class);
	
	public static void openBrowser() {
		try {
			loger.debug("opening webiste");
	...
	public static void navigate() throws InterruptedException 
	{
		try 
		{
				...
				loger.debug("Closing Popup displayed on screen");
		} catch (Exception e) {} 
		finally {
	...
	public static Boolean bidItems() throws InterruptedException, AWTException {
	...
	public static void createOrder() throws Exception {
		try {
			...
			Action openLinkInNewTab = builder.moveToElement(link).sendKeys(link, Keys.CONTROL + "t").click(link)
			...
			driver.navigate().to(Constants.CustomerSearch);
			...
			WebElement OderId = driver
					.findElement(By.xpath(".//*[@id='frm-checkout-success']/div/div/div/div[1]/div/h2/a/strong"));
	...
	public static void payment() throws Throwable {
		...
		for (String currentWindowHandle1 : winHandles1) {
			if (!currentWindowHandle1.equals(parentHandle1)) {
				...
				driver.findElement(By.id("ctl11_txtFileName")).clear();
